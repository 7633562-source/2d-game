# Copy generated crow/chicken part PNGs into Resources/Art.
# Punches near-white to alpha. Flight / sizes are not touched.

$ErrorActionPreference = "Stop"
Add-Type -AssemblyName System.Drawing

$srcDir = "C:\Users\Admin\.cursor\projects\e-Games-2D-Game-My-project\assets"
$root = "e:\Games\2D Game\My project"

$parts = @("body","neck","head","beak","tail","wing","thigh","shank","foot")
...
public static class BirdArtPunch {
...
function Write-BirdMeta([string]$pngPath) {
...
TextureImporter:
  ...
  assetBundleName:
  assetBundleVariant:
"@ | Set-Content -Path ($pngPath + ".meta") -Encoding ASCII
}

foreach ($kind in @("crow","chicken")) {
  $folder = if ($kind -eq "crow") { "Crow" } else { "Chicken" }
  $genDir = Join-Path $root "ArtIncoming\$folder\gen"
  $resDir = Join-Path $root "Assets\Resources\Art\$folder"
  New-Item -ItemType Directory -Force -Path $genDir | Out-Null
  New-Item -ItemType Directory -Force -Path $resDir | Out-Null
  foreach ($part in $parts) {
    $src = Join-Path $srcDir "${kind}_${part}.png"
    if (-not (Test-Path $src)) { throw "missing $src" }
    $dstName = "$part.png"
    $tmp = Join-Path $genDir $dstName
    [BirdArtPunch]::WhiteToAlpha($src, $tmp)
    $dst = Join-Path $resDir $dstName
    Copy-Item -Force $tmp $dst
    Write-BirdMeta $dst
    $info = New-Object System.Drawing.Bitmap $dst
    "{0}/{1} {2}x{3} A00={4}" -f $folder, $dstName, $info.Width, $info.Height, $info.GetPixel(0,0).A
    $info.Dispose()
  }
}
